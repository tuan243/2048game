...
function Tile(pos, val, puzzle) {
  ...
  this.show = function () {
    ...
    rect(
      this.getCol() * size + padding,
      this.getRow() * size + padding,
      size - 2 * padding,
      size - 2 * padding
    );
    fill(255);
  ...
  this.move = function (dir) {
    ...
    let row =
      this.getRow() + (1 - 2 * (dir < 0)) * Math.floor(Math.abs(dir) / 4);
...
function Puzzle() {
  ...
  this.getOpenPositions = () =>
    this.validPositions.filter(
      (i) => this.tiles.map((x) => x.pos).indexOf(i) === -1
    );
  ...
  this.validMoves = function () {
    ...
    for (let i = 0; i < 16; i++)
      res =
        res ||
        (i % 4 < 3 ? this.tiles[i].val === this.tiles[i + 1].val : false) ||
  ...
  this.animate = function () {
    ...
    for (let tile of this.tiles) moving = moving || tile.move(this.dir);
    ...
    if (this.hasMoved && !moving) {
      ...
      for (let tile of this.tiles) tile.merging = false;
    }
